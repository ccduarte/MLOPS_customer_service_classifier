...
def main(input_filepath, model_output_filepath):
    ...
    with Live() as live:
        ...
        live.log_metric('F1 Score', f1, timestamp=True, plot=True)

        # Salvar o modelo treinado
        joblib.dump(model, model_output_filepath)
        live.log_artifact(model_output_filepath, type="model", name="customer-service-classifier", desc="Modelo declassificação de atendimento", labels=["classificação"])
